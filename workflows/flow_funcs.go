...
func Flow1() {
	...
	for i := 0; i < 5; i++ {
		...
		for i := 1; i < int(listLen)+1; i++ {
			...
			//myList.Insert(Element{float64(i*100+1), "ELEM"+ strconv.Itoa(i*100+1)})
			myList.Insert(IntElement(i * 1000))
			//myList.Insert(Element{float64(i*100-1), "elem"+ strconv.Itoa(i*100-1)})
			//myList.Insert(Element{float64(i*100+1), "elem"+ strconv.Itoa(i*100+1)})
			fmt.Println(myList.String())
		}
...
func FLow2() {
	for i := 0; i < 5; i++ {
		reader := bufio.NewReaderSize(os.Stdin, 16*1024*1024)
		eps, err := strconv.ParseInt(strings.TrimSpace(readLine(reader)), 10, 32)

		myList := skiplist.NewEps(float64(eps))
		fmt.Println(myList)
		...
		for i := 1; i < 6; i++ {
			fmt.Println(myList.Find(IntElement(i * 1000)))
		}
	}
}
